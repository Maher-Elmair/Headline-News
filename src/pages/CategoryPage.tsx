...
function CategoryPage() {
  ...
  const metadata = categoriesData.find(c => c.slug.toLowerCase() === category.toLowerCase()) || categoriesData.find(c => c.slug === 'other');
        ...
        >
          <div className="flex items-start gap-6 mb-6">
            <div className="relative">
              <div className={`absolute inset-0 ${metadata?.color || 'text-primary'} opacity-20 blur-2xl rounded-full`} />
              <CategoryIcon className={`h-16 w-16 ${metadata?.color || 'text-primary'} relative z-10`} />
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-accent/30 rounded-xl border border-border/50">
            <div>
              ...
              {showSkeleton ? <StatValueSkeleton /> : <p className="text-2xl font-bold">{articles.length}</p>}
            </div>
            <div>
              <p className="text-sm text-muted-foreground mb-1">Updated</p>
              {showSkeleton ? <StatValueSkeleton /> : <p className="text-2xl font-bold">Today</p>}
            </div>
            <div>
              <p className="text-sm text-muted-foreground mb-1">Sources</p>
              {showSkeleton ? <StatValueSkeleton /> : <p className="text-2xl font-bold">{new Set(articles.map(a => a.author.name)).size}</p>}
            </div>
            <div>
              <p className="text-sm text-muted-foreground mb-1">Category</p>
              {showSkeleton ? <StatValueSkeleton /> : <p className="text-2xl font-bold capitalize">{categoryName}</p>}
            </div>
          </div>
        </motion.div>
